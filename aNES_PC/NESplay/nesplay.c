...
void load_nes(u8 *path)
{
    ...
    if (res == 0)
    {
        rom_file = gui_memin_malloc(f_nes->fsize); //开辟f_nes->fsize字节的内存区域
        if (rom_file)
        {
            p = rom_file;
    ...
    gui_memin_free(f_nes); //释放内存
    if (res == 0)
    {
        // 		nes_clock_set(16);			//设置为128M 超频了的 危险
        // 		delay_init(128);			//设置为128M 超频了的 危险
...
u8 nes_play(void)
{
    ...
    //	else
    //	{
    //		flistbox->fliter=FLBOX_FLT_NES;	//查找文件
    ...
    load_nes("0:/NES/超级玛丽.nes"); //开始nes游戏
                                     //				LCD_Clear(BLACK);	//清屏
